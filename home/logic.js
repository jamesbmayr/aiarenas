				...
				if (human) {
					...
					processes.retrieve("humans", {email: post.signup_email}, {}, function (human) {
						if (human) {
							...
							processes.sendEmail(null, post.signup_email, "ai_arenas human verification", "<div class='whitetext'>commence human verification process for <span class='bluetext'>" + post.signup_name + "</span>: <a class='greentext' href='https://www.aiarenas.com/verify?email=" + post.signup_email + "&verification=" + random + " '>verify</a>()</div>", function(data) {
			...
			processes.retrieve("humans", {$or: [{email: post.reset_email}, {"status.new_email": post.reset_email}]}, {}, function (human) {
				if (!human) {
					...
					processes.store("humans", {id: human.id}, {$set: {"status.verification": random, updated: new Date().getTime()}}, {}, function (human) {
						processes.sendEmail(null, (post.reset_email || null), "ai_arenas human re-verification", "<div class='whitetext'>commence human re-verification process for <span class='bluetext'>" + human.name + "</span>: <a class='greentext' href='https://www.aiarenas.com/reset?email=" + post.reset_email + "&verification=" + random + " '>reset_password</a>()</div>", function (data) {
