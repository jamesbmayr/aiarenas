...
/* update(session, post, callback) */
	function update(session, post, callback) {
		...
		if (before !== JSON.stringify(session.human)) {
			processes.store("humans", {id: session.human.id}, {$set: {settings: session.human.settings, updated: new Date().getTime()}}, {}, function (human) {
		...
		if ((typeof post.password === "undefined") || (post.password.length < 8)) {
			...
			processes.retrieve("humans",{id: session.human.id, password: processes.hash(post.current_password, session.human.salt)}, {}, function (human) {
				if (!human) {
					...
					processes.store("humans", {id: session.human.id}, {$set: {password: password, salt: salt, updated: new Date().getTime()}}, {}, function (human) {
					...
					processes.retrieve("humans",{id: session.human.id, password: processes.hash(post.current_password, session.human.salt)}, {}, function (human) {
						...
						if (!human) {
							...
							processes.store("humans", {id: session.human.id}, {$set: {"status.verification": random, "status.new_email": post.email, updated: new Date().getTime()}}, {}, function (human) {
								processes.sendEmail(null, (post.email || null), "ai_arenas human verification", "<div class='whitetext'>commence human verification process for <span class='bluetext'>" + session.human.name + "</span>: <a class='greentext' href='https://" + processes.environment("domain") + "/verify?email=" + post.email + "&verification=" + random + " '>verify</a>()</div>", function(data) {
									callback(data);
								});
							});
						}
		...
		if ((!post.session) || (post.session.length !== 32)) {
			...
			processes.retrieve("sessions", {$and: [{id: post.session}, {human: session.human.id}]}, {}, function (data) {
				if (data.human !== session.human.id) {
					...
					processes.store("sessions", {id: data.id}, {$set: {human: null, updated: new Date().getTime()}}, {}, function (result) {
